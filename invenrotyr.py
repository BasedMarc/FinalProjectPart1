...
#lists to put all the csv data in
mfl = []
prl = []
sdl = []
...
#Adding the data to the lists from each csv
# file
...
with open("ServiceDatesList.csv") as sdlist:
    pl = csv.reader(sdlist)
    for line in pl:
        prl.append(line)



#sorting each list by the order ID in order to
#get them all lining up properly
new_mfl=(sorted(mfl, key=itemgetter(0)))
new_prl=(sorted(prl, key=itemgetter(0)))
new_sdl=(sorted(sdl, key=itemgetter(0)))


#appending the missing prices and service
#dates to the main list
...
full_inventory = (sorted(final_tist, key=itemgetter(1)))

#writing the Full Inventory file with the full
#inventory list
...
#looking in each list for specific item types
#and appending them to their own lists
...
for x in range(0, len(laptop_list)):
    liwrite.writerow(taptop_list[x])
...
damagediist = (sorted(damagedlist, key=itemgetter(4), reverse=True))
